...
module.exports = HtmlAngularjsCompiler = (function () {
  ...
  HtmlAngularjsCompiler.prototype.compile = function (data, path, callback) {
    var error, parsedHtml, result, moduleName, _ref, _ref1;
    try {
      ...
      return result = ";" + moduleName + ".run(['$templateCache', function($templateCache){$templateCache.put('" + path + "', '" + parsedHtml + "')}])";
    } catch (err) {
      return error = err;
    } finally {
      ...
      _results.push((function () {
        ...
        for (_j = 0, _len1 = filePaths.length; _j < _len1; _j++) {
          ...
          if (file.path === ("" + pubDir + "/" + filePath)) {
            ...
            _results1.push(fs.writeFileSync(file.path, "var " + moduleName + "= angular.module('" + moduleName + "', [])" + data));
          } else {
